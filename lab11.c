...
int main() {
  ...
  // Read the public key. No password callback (NULL, NULL, NULL) needed.
  pubkey = PEM_read_PUBKEY(pubkey_file, NULL, NULL, NULL);
  if (!pubkey) {
    fprintf(stderr, "Error reading public key from file.\n");
    // Optionally add ERR_print_errors_fp(stderr) for detailed OpenSSL errors
    fclose(pubkey_file);
    return 1;
  }
  fclose(pubkey_file);
...
#define MAX_FILE_SIZE 512
  ...
  size_t message_len, signature_len;
  EVP_MD_CTX *mdctx = NULL;
  int result = -1;

  message_len = read_all_bytes(message_path, message, MAX_FILE_SIZE);
  signature_len = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);
