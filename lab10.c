...
- the client is using TCP connection (AF_INET, SOCK_STREAM) to send data to the
server over a reliable byte-stream socket.

2. What data is the client sending to the server?
- the client send some short text strings ("Hello","Apple","Car","Green", "Dog")
  every message is written as a 1024-byte buffer.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
- The `run_acceptor` thread gets a pointer to an acceptor_args struct.
  This struct contains a pointer to the shared linked-list handle, a mutex
  protecting the list, and a run flag. The acceptor thread uses these to
  accept client connections, create client threads, and coordinate shutdown
  with main().

2. How are received messages stored?
- Each received message is stored as a node in a linked list. When a client
  thread receives a message, it creates a new node containing wit the recieved
  message string and appends it to the end of the list using add_to_list(), a
  mutex is used to keep access to the list thread-safe.

3. What does `main()` do with the received messages?
- main() waits until the number of received messages reaches
  MAX_CLIENTS * NUM_MSG_PER_CLIENT. It then stops the acceptor thread,
  joins it, and finally calls collect_all() to print and free all the
  stored messages.

4. How are threads used in this sample code?
- There is one acceptor thread responsible for accepting client connections.
  For each connected client, a new client thread is created that receives
  messages and stores them in the shared list. main() remains in control
  and shuts everything down once enough messages have been collected.
*/
